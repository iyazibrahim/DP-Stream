...
async function videoRoutes(fastify) {
  fastify.get('/', { preHandler: [fastify.requireAuth] }, async (request, reply) => {
    ...
    const [videos] = await fastify.db.execute(
      `SELECT v.id, v.title, v.description, v.thumbnail_path, v.status, v.published_at,
              vp.completed_at,
              (
                SELECT c.id
       ...
       WHERE v.status = "published"
         AND (? = ''
              ...
              OR LOWER(IFNULL(v.description, '')) LIKE ?
              OR EXISTS (
                SELECT 1
                FROM course_videos cv
                INNER JOIN courses c ON c.id = cv.course_id
         ...
         AND (? = 0 OR EXISTS (
              SELECT 1
              FROM course_videos cv2
              WHERE cv2.video_id = v.id
                AND cv2.course_id = ?
         ))
       ORDER BY CASE WHEN v.display_order > 0 THEN v.display_order ELSE 2147483647 END ASC,
                v.published_at DESC, v.created_at DESC`,
      [request.user.sub, query, likeQuery, likeQuery, likeQuery, courseId, courseId]
    );

    const [courses] = await fastify.db.execute(
      `SELECT c.id, c.name, c.description, c.thumbnail_path,
              COUNT(DISTINCT CASE WHEN v.status = 'published' THEN v.id END) AS published_count
       FROM courses c
       LEFT JOIN course_videos cv ON cv.course_id = c.id
       LEFT JOIN videos v ON v.id = cv.video_id
       WHERE (? = ''
          OR LOWER(c.name) LIKE ?
          OR LOWER(IFNULL(c.description, '')) LIKE ?)
       GROUP BY c.id
    ...
    return reply.view('videos/list.ejs', {
      user: request.user,
      videos,
      courses,
      selectedCourse,
